...
| `new_engagement.py`  | Scaffold a new pentest engagement directory under `./engagements/<client>/<date>/` with report.md template, engagement.yaml, timeline.md, hosts.csv, and an empty `evidence/`. Invoked by the `report-writer-internalpen` agent at engagement kickoff. |
...
| `htb_api.py` | HTB API v4 helper — active machine info, spawn/stop/reset, flag submission, KB cross-reference. Subcommands: `active`, `info <name>`, `search <term>`, `spawn <name>`, `stop`, `reset`, `flag <value>`, `hints`. Reads `HTB_API_KEY` from `.env`. Requires `requests`. |
| `ssh_cmd.py` | Generic password-auth SSH command runner (paramiko). Like `kali_ssh.py` but fully positional — `ssh_cmd.py <host> <user> <pass> "<cmd>"` (or cmd via stdin). For pivoting to lab targets whose creds are recovered mid-engagement. No hardcoded config. |
| `render_htb_writeup.py` | Render an HTB `writeups/htb/<slug>/writeup.md` (YAML-ish frontmatter + markdown) into a self-contained `writeup.html` styled like the p3ta00 CTF blog (JetBrains Mono "Cyberpunk Neon" terminal theme, JFM ASCII masthead, base64-inlined screenshots, Pygments highlighting). Used by the `report-writer-htb` agent. Requires `markdown` + `pygments`. |
